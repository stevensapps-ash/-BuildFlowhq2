...
function Invoices({data,setData}:{data:AppData;setData:(d:any)=>void}){
  ...
  function add(){const cleanCustomer=String(customer||'').trim();const validItems=items.filter(x=>String(x.description||'').trim()&&Number(x.qty)>0&&Number(x.rate)>=0);if(!cleanCustomer){alert('Customer is required.');return}if(!validItems.length){alert('Add at least one valid invoice line item.');return}if(total<=0){alert('Invoice total must be greater than $0.');return}const id=Date.now();const invoice={id,number:'INV-'+String(id).slice(-6),project:String(project||'').trim(),customer:cleanCustomer,items:validItems.map(x=>({...x,description:String(x.description||'').trim(),qty:Number(x.qty),rate:Number(x.rate)})),subtotal,discount:discountAmount,taxRate:Number(taxRate||0),tax,amount:total,balance:status==='Paid'?0:total,due,status,notes:String(notes||'').trim(),createdAt:localDate()};setData({...data,invoices:[invoice,...(data.invoices||[])],docs:[{id:id+1,type:'Invoice',project:invoice.project,customer:invoice.customer,status:invoice.status,invoiceId:id,createdAt:localDate()},...(data.docs||[])]});setProject('');setCustomer('');setDue('');setStatus('Draft');setTaxRate('0');setDiscount('0');setNotes('');setItems([{id:Date.now()+2,description:'',qty:1,rate:0}])}
