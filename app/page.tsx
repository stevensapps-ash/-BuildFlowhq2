...
function Invoices({data,setData}:{data:AppData;setData:(d:any)=>void}){
  const[project,setProject]=useState(''),[customer,setCustomer]=useState(''),[due,setDue]=useState(''),[status,setStatus]=useState('Draft'),[taxRate,setTaxRate]=useState('0'),[discount,setDiscount]=useState('0'),[notes,setNotes]=useState(''),[items,setItems]=useState<any[]>([{id:Date.now(),description:'',qty:1,rate:0}])
  ...
  function add(){if(!customer.trim()||!items.some(x=>x.description.trim()&&Number(x.qty)>0&&Number(x.rate)>=0)||total<=0)return;const id=Date.now();const invoice={id,number:'INV-'+String(id).slice(-6),project:project.trim(),customer:customer.trim(),items:items.filter(x=>x.description.trim()).map(x=>({...x,qty:Number(x.qty),rate:Number(x.rate)})),subtotal,discount:discountAmount,taxRate:Number(taxRate||0),tax,amount:total,balance:status==='Paid'?0:total,due,status,notes:notes.trim(),createdAt:localDate()};setData({...data,invoices:[invoice,...data.invoices],docs:[{id:id+1,type:'Invoice',project:invoice.project,customer:invoice.customer,status:invoice.status,invoiceId:id,createdAt:localDate()},...data.docs]});setProject('');setCustomer('');setDue('');setStatus('Draft');setTaxRate('0');setDiscount('0');setNotes('');setItems([{id:Date.now()+2,description:'',qty:1,rate:0}])}
  function printInvoice(x:any){const rows=(x.items||[{description:x.project||'Services',qty:1,rate:x.amount}]).map((i:any)=>'<tr><td>'+String(i.description||'')+'</td><td>'+Number(i.qty||1)+'</td><td>'+money(Number(i.rate||0))+'</td><td>'+money(Number(i.qty||1)*Number(i.rate||0))+'</td></tr>').join('');const w=window.open('','_blank');if(!w)return;w.document.write('<html><head><title>'+String(x.number||'Invoice')+'</title><style>body{font-family:Arial;padding:40px;color:#111}h1{margin-bottom:4px}table{width:100%;border-collapse:collapse;margin-top:24px}th,td{padding:10px;border-bottom:1px solid #ddd;text-align:left}.totals{text-align:right;margin-top:24px;font-size:18px}</style></head><body><h1>INVOICE</h1><b>'+String(data.settings.businessName||'Construction HQ')+'</b><p>'+String(x.number||'')+' · '+String(x.createdAt||'')+'</p><h3>Bill to: '+String(x.customer||'')+'</h3><p>'+String(x.project||'')+'</p><table><thead><tr><th>Description</th><th>Qty</th><th>Rate</th><th>Total</th></tr></thead><tbody>'+rows+'</tbody></table><div class="totals"><p>Subtotal: '+money(Number(x.subtotal??x.amount))+'</p>'+(Number(x.discount||0)?'<p>Discount: -'+money(Number(x.discount))+'</p>':'')+(Number(x.tax||0)?'<p>Tax: '+money(Number(x.tax))+'</p>':'')+'<h2>Total: '+money(Number(x.amount||0))+'</h2></div><p>Due: '+String(x.due||'Upon receipt')+'</p><p>'+String(x.notes||'')+'</p></body></html>');w.document.close();w.focus();setTimeout(()=>w.print(),250)}
  return <div className="workspace"><Head title="Invoice Maker" text="Build itemized invoices, calculate totals, save them to the customer record, and print or save as PDF."/><div className="builderGrid">
    ...
    <Field label="Discount $" value={discount} set={setDiscount}/><Field label="Tax %" value={taxRate} set={setTaxRate}/><label style={{gridColumn:'1/-1'}}>Invoice notes<textarea value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Payment terms, thank-you note, scope details…"/></label><div><b>Subtotal {money(subtotal)}</b><br/><span>Tax {money(tax)} · Total {money(total)}</span></div><button className="primary" onClick={add}><Plus/>Save Invoice</button>
  </div>
  {data.invoices.length?data.invoices.map((x:any)=><div className="wideRow" key={x.id}><Receipt size={18}/><div className="grow"><b>{x.number?x.number+' · ':''}{x.customer}{x.project?' · '+x.project:''}</b><span>{x.due?'Due '+x.due:'Due upon receipt'} · {x.status||'Draft'}</span></div><strong>{money(x.amount)}</strong><select aria-label="Invoice status" value={x.status||'Draft'} onChange={e=>{const s=e.target.value;setData({...data,invoices:data.invoices.map((y:any)=>y.id===x.id?{...y,status:s,balance:s==='Paid'?0:y.amount}:y)})}}><option>Draft</option><option>Sent</option><option>Overdue</option><option>Paid</option></select><button className="primary" onClick={()=>printInvoice(x)}>PDF / Print</button><button className="iconButton" title="Delete invoice" onClick={()=>setData({...data,invoices:data.invoices.filter((y:any)=>y.id!==x.id)})}><Trash2 size={15}/></button></div>):<p>No invoices yet.</p>}</div>
}
