...
function Invoices({data,setData}:{data:AppData;setData:(d:any)=>void}){
  ...
  function add(){if(!customer.trim()){alert('Customer is required.');return}if(!items.some(x=>x.description.trim()&&Number(x.qty)>0&&Number(x.rate)>=0)){alert('Add at least one valid invoice line item.');return}if(total<=0){alert('Invoice total must be greater than $0.');return}const id=Date.now();const invoice={id,number:'INV-'+String(id).slice(-6),project:project.trim(),customer:customer.trim(),items:items.filter(x=>x.description.trim()).map(x=>({...x,qty:Number(x.qty),rate:Number(x.rate)})),subtotal,discount:discountAmount,taxRate:Number(taxRate||0),tax,amount:total,balance:status==='Paid'?0:total,due,status,notes:notes.trim(),createdAt:localDate()};setData({...data,invoices:[invoice,...data.invoices],docs:[{id:id+1,type:'Invoice',project:invoice.project,customer:invoice.customer,status:invoice.status,invoiceId:id,createdAt:localDate()},...data.docs]});setProject('');setCustomer('');setDue('');setStatus('Draft');setTaxRate('0');setDiscount('0');setNotes('');setItems([{id:Date.now()+2,description:'',qty:1,rate:0}])}
